    ...
    <div class="absolute bg-black/[0.3] inset-0 flex justify-center items-center">
      <div class="bg-white w-96 flex justify-center flex-col">
          <div class="p-4 flex justify-between border-b">
                ...
                <button @click="closeModal(); createEntry.reset()" class="text-xl bold">x</button>
          </div>
          <div class="px-4 pb-4">
            <div class="flex flex-col mb-4 mt-2">
                ...
                <p class="text-red-500 invisible"
                   :class="[createEntry?.error?.response?.data?.errors?.name ? '!visible' : '!invisible']">
                    Name is required
                </p>
            </div>
            ...
            <div class="flex flex-col mb-4">
                ...
                <p class="text-red-500 invisible"
                   :class="[createEntry?.error?.response?.data?.errors?.calories  ? '!visible' : '!invisible']">
            ...
            <div class="flex flex-col mb-4">
                ...
                <p class="text-red-500 invisible"
                   :class="[createEntry?.error?.response?.data?.errors?.price  ? '!visible' : '!invisible']">
                    Price is required
                </p>
            </div>
            <div class="flex justify-end">
                <button @click="closeModal(); createEntry.reset()" class="bg-red-500 text-white rounded p-2 mr-2">Close</button>
